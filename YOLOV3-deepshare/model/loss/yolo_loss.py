...
class YoloV3Loss(nn.Module):
    ...
    def forward(self, p, p_d, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes):
        ...
        loss = loss_l + loss_m + loss_s #三个尺度上总的loss
        loss_giou = loss_s_giou + loss_m_giou + loss_l_giou #三个尺度上giou的loss
        loss_conf = loss_s_conf + loss_m_conf + loss_l_conf #三个尺度上conf loss
        loss_cls = loss_s_cls + loss_m_cls + loss_l_cls #三个尺度上cls loss
    ...
    def __cal_loss_per_layer(self, p, p_d, label, bboxes, stride):
        ...
        loss = loss_giou + loss_conf + loss_cls # 三个losses的综合是loss
