...
class VocDataset(Dataset):
    def __init__(self, anno_file_type, img_size=416):
        ...
        self.classes = cfg.DATA["CLASSES"] # VOC 20类
        self.num_classes = len(self.classes) # 20
        self.class_to_id = dict(zip(self.classes, range(self.num_classes))) # 将编号和class一一对应
        self.__annotations = self.__load_annotations(anno_file_type)

    def __len__(self):
        return len(self.__annotations)  # 总长度，因为之后要按照batch进行切分
    ...
    def __getitem__(self, item):
        ...
        img, bboxes = dataAug.Mixup()(img_org, bboxes_org, img_mix, bboxes_mix) # 图片的random融合
    ...
    def __load_annotations(self, anno_type):
        ...
        anno_path = os.path.join(cfg.PROJECT_PATH, 'data', anno_type+"_annotation.txt") # 刚才VOC生成的annotations文件
        # 去掉空的行
    ...
    def __parse_annotation(self, annotation):
        ...
        img_path = anno[0] # anno中的第一个元素是图片的路径
    ...
    def __creat_label(self, bboxes):
        ...
        1、Select a bbox in order, convert its coordinates("xyxy") to "xywh"; and scale bbox'
           xywh by the strides. 坐标转换，并根据stride缩放bbox
        ...
        for bbox in bboxes:
            bbox_coor = bbox[:4] # coordinate
            bbox_class_ind = int(bbox[4])
            bbox_mix = bbox[5]

            # onehot
            one_hot = np.zeros(self.num_classes, dtype=np.float32)
            # 读取cls的类别，并将其附在我们的格式中,这里我们叫他one-hot的原因是我们将cls一个个拿出来进行数据的写入
            one_hot[bbox_class_ind] = 1.0
            # 上面一行得到one-hot之后再进行标签平滑
            ...
            # 8/16/32 三个stride，得到不同scale的Ground Truth box
